...
import static SblPatNodeType.*
import static CompiledState.*
import static SblPrimitives.*
...
//                nNode.context.registerPattern( nNode.name, nNode)
    ...
    StringBuilder getCompactPatString() {
        StringBuilder sb = new StringBuilder()
        switch (type) {
        case ROOT:
            // should only be one child for ROOT
            sb.append( children[0].getCompactPatString())
            break

        case CONCATENATE:
            // compact concatenate the strings of the children
            // if both children are StringPat nodes, just concatenate
            // otherwise, wrap in parens and add a '+'
            if( children[0].type == STRINGPAT && children[1].type == STRINGPAT){
                sb.append( children[0].getPatString()).append( children[1].getPatString())
            } else {
                sb = new StringBuilder(getPatString())
            }
            break
